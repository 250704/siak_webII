...
class MataPelajaranController extends Controller
{
    ...
    {
        $mataPelajarans = MataPelajaran::with('gurus')->paginate(10);
    ...
    public function store(Request $request)
    {
        $validated = $request->validate([
            'nama_matpel' => 'required|string|max:255|unique:mata_pelajarans',
        ]);
    ...
    {
        $validated = $request->validate([
            'nama_matpel' => 'required|string|max:255|unique:mata_pelajarans,nama_matpel,' . $mataPelajaran->id,
        ]);
